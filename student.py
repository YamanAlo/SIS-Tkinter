...
class StudentListWindow(ctk.CTkToplevel):
    ...
    def show_students_list(self):
        ...
        for student in students:
            label_text = f"Student ID: {student[0]},First Name: {student[1]}, Last Name: {student[2]}, Email: {student[3]}, Phone: {student[4]}, Address: {student[5]}, City: {student[6]}"
    ...
    def show_context_menu(self, event, student_id):
        ...
        context_menu.add_command(label="Edit", command=lambda: self.edit_student(student_id))
        context_menu.add_command(label="Delete", command=lambda: self.delete_student(student_id))
    ...
    def edit_student(self, student_id):
        ...
        if selected_student:
            # Store the currently selected student_id
            ...
            # Student ID
            student_id_label = ctk.CTkLabel(entry_frame, text="Student ID:")
            ...
            # First Name
            First_name_label = ctk.CTkLabel(entry_frame, text="First Name:")
            ...
            # Last Name
            Last_name_label = ctk.CTkLabel(entry_frame, text="Last Name:")
            ...
            # email
            student_email_label = ctk.CTkLabel(entry_frame, text="Email:")
            ...
            # phone
            student_phone_label = ctk.CTkLabel(entry_frame, text="Phone:")
            ...
            # address
            student_address_label = ctk.CTkLabel(entry_frame, text="Address:")
            ...
            # city
            student_city_label = ctk.CTkLabel(entry_frame, text="City:")
    ...
    def save_changes(self, edit_window, student_id, first_name, last_name, email, phone, address, city):
        ...
        if not student_id or not first_name or not last_name or not email or not phone or not address or not city:
            msg.CTkMessagebox(title = "Error", message = "Please enter all the required fields" , icon = "cancel")
            return
        ...
        if any(char.isdigit() for char in first_name + last_name + email + address + city):
            ...
            error_message = f"{', '.join(error_fields)} should not contain numbers."
            msg.CTkMessagebox(title = "Error", message = error_message , icon = "cancel")
            return
        
        if not student_id.isdigit():
            msg.CTkMessagebox(title = "Error", message= "Student ID should be a number.", icon = "cancel")
            return
        
        if not phone.isdigit():
            msg.CTkMessagebox(title = "Error", message="Phone number should be a number.",icon = "cancel")
            return
        

        if not first_name.isalpha():
            msg.CTkMessagebox(title = "Error", message= "First name should not include numbers.", icon = "cancel")
            return
        
        if not last_name.isalpha():
            msg.CTkMessagebox(title = "Error", message="Last name should not include numbers.", icon = "cancel")
            return
        
        if not all(char.isalnum() or char in ['@', '.'] for char in email):
            msg.CTkMessagebox(title ="Error", message= "Email should contain only alphabets, numbers, '@', and '.'", icon = "cancel")
            return
        

        try:
            self.db.update_student(student_id, first_name, last_name, email, phone, address, city)
            msg.CTkMessagebox(title = "Success", message= "Student updated successfully.", icon = "check" , option_1="Thanks")
            self.show_students_list()  # Refresh the student list after update
            edit_window.destroy()  # Close the edit window
        except Exception as e:
          msg.CTkMessagebox(title= "Error", message= "Failed to update student information:", icon = "cancel")

    def delete_student(self, student_id):
       
            try:
                self.db.delete_student(student_id)
                msg.CTkMessagebox(title = "Confirm Deletion", message= "Are you sure you want to delete this student?", icon="question", option_1="Cancel", option_2="No", option_3="Yes")
                self.show_students_list()  # Refresh the course list after deletion
            except Exception as e:
                msg.CTkMessagebox(title = "Error", message= "Failed to remove student ", icon="cancel")
...
class StudentManagementWindow(ctk.CTkToplevel):
    ...
    def add_student(self):
        ...
        if any(char.isdigit() for char in student_first_name + student_last_name +  student_address + student_city):
            error_fields = [field for field, value in {"First Name": student_first_name, "Last Name": student_last_name, "Email": student_email, "Address": student_address, "City": student_city}.items() if any(char.isdigit() for char in value)]
            error_message = f"{', '.join(error_fields)} should not contain numbers."
            msg.CTkMessagebox(title="Error",message= error_message , icon="cancel")
            return

        # add validition for student id and phone number
        if not student_id.isdigit():
            msg.CTkMessagebox(title= "Error",message= "Student ID should be a number." , icon="cancel")
            return
        ...
        # first name and lastname shouldnt include characters 
        ...
        try:
            ...
            msg.CTkMessagebox(title="Success", message= "Student added successfully.", icon="check", option_1="Thanks")
        except Exception as e:
            msg.CTkMessagebox(title="Error", message= "Failed to add student.", icon="cancel")
